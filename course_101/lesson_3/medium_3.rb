...
# Why? It seems clear from the above that Ruby treats string and array parameters differently...but not so fast.
...
# the value that gets passed is a reference to some object. The string parameter is passed to the method as a reference
# to an object of type String. Similarly, the array is passed to the method as a reference to an object of type Array.
# The important distinction is that while a reference is passed, the method makes up a new variable for
# both the string and the array, and puts these references in these new variables...
# these are variables that only live within the scope of the method.
# So if both parameters live inside the method as a new variable that stores a reference to the passed object...
...
# The reference to this new object is placed in the internal variable of the method,
# and the method considers this new string object to be the one true string. It has forgotten about the one that was passed in now.
# That old stale parameter string is still remembered by the caller of the method..
# but the method itself has forgotten all about it.
...
# # So, because the method's array variable still points to the original object, the caller's array variable and the method's
# array variable are still pointing at the same object,
...
# We are still passing in the same two string and array parameters, but the results are the opposite,
# because our method does the "same thing" to each, but in a different way than before.
# That is, in both cases, the method attempts to change the string from pumpkins to rutabaga and add a rutabaga to the array.
# Despite the similarity of results INSIDE the method, the results on the outside of the method are the opposite.
# As before, the explanation is all about what Ruby does with its internal variables when we change the object it points to.
...
# and we are setting the internal array variable equal to that new array literal object.
